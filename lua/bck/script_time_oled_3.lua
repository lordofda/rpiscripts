...
if (time.min % 3 == 1) then
    ...
    commandArray[6]={['OpenURL']=Adres..'/control?cmd=oled,4,1,----------------'}
    commandArray[7]={['OpenURL']=Adres..'/control?cmd=oled,8,1,----------------'}
end
if (time.min % 3 == 2) then
    commandArray[1]={['OpenURL']=Adres..'/control?cmd=oledcmd,clear'}
    commandArray[2]={['OpenURL']=Adres..'/control?cmd=oled,1,1,Z:'..avg1}
    commandArray[3]={['OpenURL']=Adres..'/control?cmd=oled,1,10,W:'..avg}

    commandArray[4]={['OpenURL']=Adres..'/control?cmd=oled,2,1,----------------'}

    commandArray[5]={['OpenURL']=Adres..'/control?cmd=oled,3,1,CO:'}
    commandArray[6]={['OpenURL']=Adres..'/control?cmd=oled,4,1,PM10:'}
    commandArray[7]={['OpenURL']=Adres..'/control?cmd=oled,6,1,NO2:'}
    commandArray[8]={['OpenURL']=Adres..'/control?cmd=oled,7,1,SO2:'}

    if not (otherdevices_svalues['CO'] == "brak danych") then
        commandArray[9]={['OpenURL']=Adres..'/control?cmd=oled,3,7,'..round(otherdevices_svalues['CO'],0)}
    else
	commandArray[9]={['OpenURL']=Adres..'/control?cmd=oled,3,7,---'}
    end
    if not (otherdevices_svalues['PM10'] == "brak danych") then
        commandArray[10]={['OpenURL']=Adres..'/control?cmd=oled,4,7,'..round(otherdevices_svalues['PM10'],0)}
    else
        commandArray[10]={['OpenURL']=Adres..'/control?cmd=oled,4,7,---'}
    end
    if not (otherdevices_svalues['NO2'] == "brak danych") then
        commandArray[11]={['OpenURL']=Adres..'/control?cmd=oled,6,7,'..round(otherdevices_svalues['NO2'],0)}
    else
        commandArray[11]={['OpenURL']=Adres..'/control?cmd=oled,6,7,---'}
    end
    if not (otherdevices_svalues['SO2'] == "brak danych") then
        commandArray[12]={['OpenURL']=Adres..'/control?cmd=oled,7,7,'..round(otherdevices_svalues['SO2'],0)}
    else
        commandArray[12]={['OpenURL']=Adres..'/control?cmd=oled,7,7,---'}
    end	

    commandArray[13]={['OpenURL']=Adres..'/control?cmd=oled,3,13,10k'}
    commandArray[14]={['OpenURL']=Adres..'/control?cmd=oled,4,14,50'}
    commandArray[15]={['OpenURL']=Adres..'/control?cmd=oled,6,13,200'}
    commandArray[16]={['OpenURL']=Adres..'/control?cmd=oled,7,13,350'}
